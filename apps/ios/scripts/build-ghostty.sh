...
# [litter-fork] Keep a pre-populated zig package cache (set GHOSTTY_KEEP_ZIG_CACHE=1)
# so an offline/China-network build can reuse curl-prefetched deps instead of
# letting zig's (proxy-broken) fetcher re-download them on every run.
