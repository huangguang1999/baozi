...
final class WatchCompanionBridge: NSObject {
    ...
    private static let appGroupSuite = "group.com.kris99.baozi"
    ...
    private func observeHomePreferencesChanges() {
        ...
        preferencesObserver = NotificationCenter.default.addObserver(
            forName: .baoziThreadPreferencesDidChange,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            Task { @MainActor in
    ...
    /// Build a `BaoziComplicationPayload` Data slice per known server. The
    ...
    /// reconnects.
    func currentServerListPayload() -> BaoziServerListPayload {
        let servers = (AppModel.shared.snapshot?.servers ?? []).map {
            BaoziServerListPayload.Server(
                id: $0.serverId,
                displayName: $0.displayName
            )
        }
        return BaoziServerListPayload(servers: servers)
    }
