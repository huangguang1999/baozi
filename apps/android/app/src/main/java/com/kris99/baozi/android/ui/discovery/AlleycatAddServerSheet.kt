...
import com.kris99.baozi.android.ui.common.isBetaAgentName
    ...
    Column(
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            TextButton(onClick = onDismiss, enabled = !isConnecting) {
                Text("取消", color = BaoziTheme.accent)
            }
        }
        ...
        if (showPaste) {
            ...
            Row(
                ...
                TextButton(
                    ...
                    Text("从剪贴板粘贴", color = BaoziTheme.accent)
                }
                TextButton(
        ...
        parseError?.let { message ->
            Text(message, color = BaoziTheme.warning, fontSize = 12.sp)
        }
            ...
            Column(
                ...
                when {
                    isLoadingAgents -> Row(
                        ...
                        Spacer(Modifier.width(8.dp))
                        Text("正在加载智能体", color = BaoziTheme.textSecondary, fontSize = 12.sp)
                    }
                    agents.isEmpty() -> Text(
                        text = "此主机上没有可用的智能体。",
                        color = BaoziTheme.textMuted,
        ...
        Button(
            ...
            colors = ButtonDefaults.buttonColors(
                containerColor = BaoziTheme.accent.copy(alpha = 0.18f),
        ...
        connectError?.let { message ->
            Text(message, color = BaoziTheme.danger, fontSize = 12.sp)
        }
    }
}
...
private fun AgentRow(
    ...
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text(
                    ...
                    color = if (agent.available) BaoziTheme.textPrimary else BaoziTheme.textMuted,
                    fontSize = 13.sp,
        ...
        if (!agent.available) {
            Text("不可用", color = BaoziTheme.textMuted, fontSize = 11.sp)
        } else {
            Checkbox(
                checked = selected,
...
private const val PAIR_COMMAND = "npx baozicli"
...
private fun StepRow(number: String, title: String) {
    Row(
        ...
        Box(
            modifier = Modifier
                .size(20.dp)
                .background(BaoziTheme.accent, androidx.compose.foundation.shape.CircleShape),
        ...
        TextButton(
            onClick = {
                ...
                clipboard?.setPrimaryClip(
                    android.content.ClipData.newPlainText("包子", PAIR_COMMAND),
                )
                copied = true
                scope.launch {
