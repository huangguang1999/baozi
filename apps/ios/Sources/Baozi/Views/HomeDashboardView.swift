...
struct HomeDashboardView: View {
    ...
    private var dashboardBackground: some View {
        if BaoziPlatform.rendersAsMacApp && chrome == .sidebar {
            Color.clear
        } else {
    ...
    private var sidebarBottomChrome: some View {
        ...
        .background(
            LinearGradient(
                colors: Array(BaoziTheme.headerScrim.reversed()),
                startPoint: .top,
                endPoint: .bottom
            )
            .padding(.top, -30)
...
struct SessionCanvasLine: View {
    ...
    private var toolLogFontSize: CGFloat {
        max(12, BaoziFont.conversationBodyPointSize - 3)
    }
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 6) {
                    FormattedText(text: session.sessionTitle, lineLimit: zoomLevel >= 4 ? 4 : 2)
                        ...
                        .foregroundStyle(isActive ? BaoziTheme.accent : BaoziTheme.textPrimary)
                    ...
                    if zoomLevel == 1 {
                        Text(timeAgo)
                            ...
                            .foregroundStyle(BaoziTheme.textMuted.opacity(0.7))
                            .fixedSize()
                    }
                }
                .frame(maxWidth: .infinity, alignment: .leading)
        ...
        .background(alignment: .leading) {
            if isActive {
                BaoziTheme.accent.opacity(0.3).frame(width: 2)
            }
        }
        .background(isActive ? BaoziTheme.accent.opacity(0.02) : Color.clear)
    ...
    private var metaLine: some View {
        HStack(spacing: 4) {
            Text(timeAgo)
                .foregroundStyle(BaoziTheme.textMuted.opacity(0.8))
            ...
            if isActive && isToolCallRunning {
                Text("\u{00b7}")
                    .foregroundStyle(BaoziTheme.textMuted.opacity(0.5))
                toolActivityLabel
                SessionPulsingDots()
                statChips
            } else {
                ...
                Text(session.serverDisplayName)
                    .foregroundStyle(BaoziTheme.textSecondary.opacity(0.7))
                if let workspace = HomeDashboardSupport.workspaceLabel(for: session.cwd) {
                    Text("\u{00b7}")
                        .foregroundStyle(BaoziTheme.textMuted.opacity(0.5))
                    Text(workspace)
                        .foregroundStyle(BaoziTheme.textSecondary.opacity(0.8))
                }
                statChips
            }
        }
    ...
    private var statChips: some View {
        ...
        if let tu = session.tokenUsage, let window = tu.contextWindow, window > 0 {
            ...
            RollingMetricText("\(pct)%")
                .foregroundStyle(pct > 80 ? BaoziTheme.warning.opacity(0.8) : BaoziTheme.textMuted.opacity(0.8))
        }
    }
    ...
    private var modelBadgeLine: some View {
        HStack(spacing: 4) {
            ...
            Image(systemName: "server.rack")
                .baoziFont(size: 8)
                .foregroundStyle(BaoziTheme.accent.opacity(0.5))
            Text(session.serverDisplayName)
                .foregroundStyle(BaoziTheme.accent.opacity(0.6))
            let m = session.model.trimmingCharacters(in: .whitespacesAndNewlines)
            if !m.isEmpty {
                Text("\u{00b7}").foregroundStyle(BaoziTheme.textMuted.opacity(0.5))
            ...
            if let lineage = session.lineage, lineage.hasMultipleBranches {
                ...
                Text("\u{00b7}").foregroundStyle(BaoziTheme.textMuted.opacity(0.5))
                Text("fork")
                    .foregroundStyle(BaoziTheme.warning.opacity(0.8))
            }
        ...
        if hasAny {
            HStack(spacing: 12) {
                ...
                if let stats, hasDiff {
                    HStack(spacing: 5) {
                        ...
                        RollingMetricText("-\(stats.diffDeletions)")
                            .foregroundStyle(BaoziTheme.danger.opacity(0.65))
                    }
                }
                ...
                if let tu = session.tokenUsage, let window = tu.contextWindow, window > 0 {
                    let pct = Int((Double(tu.totalTokens) / Double(window)) * 100)
                    RollingMetricText("\(pct)%")
                        .foregroundStyle(pct > 80 ? BaoziTheme.warning.opacity(0.85) : BaoziTheme.textMuted.opacity(0.75))
                }
                Spacer(minLength: 0)
            }
            .baoziMonoFont(size: 10, weight: .regular)
    ...
    private var telemetryDashboard: some View {
        ...
        if hasAny {
            VStack(alignment: .leading, spacing: 0) {
                if let goal {
                    ...
                    if hasMetrics {
                        ...
                        Rectangle()
                            .fill(BaoziTheme.border.opacity(0.4))
                            .frame(height: 0.5)
                            .padding(.vertical, 8)
                    }
                }
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack(alignment: .top, spacing: 14) {
                            statCell(icon: "chevron.left.forwardslash.chevron.right",
                                     ...
                                     valueColor: BaoziTheme.textPrimary,
                                     label: "files")
                            statCell(icon: nil,
                                     valuePrefix: nil,
                                     value: pct.map { "\($0)%" },
                                     valueColor: (pct ?? 0) > 80 ? BaoziTheme.warning : BaoziTheme.warning.opacity(0.85),
                                     label: "context")
                        }
                        HStack(alignment: .top, spacing: 14) {
                            statCell(icon: nil,
                                     value: adds > 0 ? "+\(adds.formatted(.number.grouping(.automatic)))" : nil,
                                     valueColor: BaoziTheme.accent,
                                     label: "added")
                        ...
                        HStack(alignment: .top, spacing: 14) {
                            statCell(icon: nil,
                                     ...
                                     valueColor: BaoziTheme.danger.opacity(0.85),
                                     label: "removed")
                            statCell(icon: "clock",
            ...
            .overlay(alignment: .top) {
                Rectangle().fill(BaoziTheme.border.opacity(0.5))
                    .frame(height: 0.5)
            }
            .overlay(alignment: .bottom) {
                Rectangle().fill(BaoziTheme.border.opacity(0.5))
    ...
    private func goalBanner(goal: AppThreadGoal) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: 6) {
                Text("GOAL")
                    .baoziMonoFont(size: 9, weight: .semibold)
                    .tracking(1.2)
                    .foregroundStyle(BaoziTheme.textMuted.opacity(0.65))
            ...
            HStack(alignment: .top, spacing: 8) {
                ...
                Text(goal.objective)
                    .baoziMonoFont(size: 12, weight: .regular)
                    .foregroundStyle(BaoziTheme.textSecondary.opacity(0.95))
                    .lineLimit(2)
                    .truncationMode(.tail)
                    .frame(maxWidth: .infinity, alignment: .leading)
            }
    ...
    private var goalLine: some View {
        if let goal = session.goal {
            HStack(spacing: 6) {
                ...
                Text(goal.objective)
                    .foregroundStyle(BaoziTheme.textSecondary.opacity(0.85))
                    .lineLimit(1)
                    .truncationMode(.tail)
                Spacer(minLength: 6)
    ...
    private func goalStatusTint(_ status: AppThreadGoalStatus) -> Color {
        switch status {
        case .active: return BaoziTheme.accent
        case .paused: return BaoziTheme.textMuted
        case .blocked, .usageLimited, .budgetLimited: return BaoziTheme.warning
        case .complete: return BaoziTheme.success
        }
    }
    ...
    private var userMessageLine: some View {
        ...
        if !message.isEmpty && message != title {
            ...
            FormattedText(text: message, lineLimit: zoomLevel >= 4 ? 3 : 1)
                .foregroundStyle(BaoziTheme.textSecondary.opacity(0.95))
                .baoziFont(size: BaoziFont.conversationBodyPointSize)
                .frame(maxWidth: .infinity, alignment: .leading)
                .padding(.vertical, 5)
                .padding(.leading, 8)
                .padding(.trailing, 6)
                .background(BaoziTheme.accent.opacity(0.06))
                .overlay(alignment: .leading) {
                    BaoziTheme.accent.opacity(0.55).frame(width: 2)
                }
                .clipShape(
        ...
        HStack(spacing: 8) {
            ...
            if isActive {
                HStack(spacing: 5) {
                    ...
                    Text("Working")
                        .baoziMonoFont(size: 9, weight: .semibold)
                        .foregroundStyle(BaoziTheme.accent.opacity(0.85))
                }
                .padding(.horizontal, 7)
                .padding(.vertical, 2)
                .overlay(
                    Capsule()
                        .stroke(BaoziTheme.accent.opacity(0.4), lineWidth: 0.5)
                )
                .clipShape(Capsule())
                .fixedSize()
            }
    ...
    private func toolRowView(_ entry: AppToolLogEntry) -> some View {
        ...
        .baoziFont(size: toolLogFontSize)
        .frame(maxWidth: .infinity, alignment: .leading)
    }
    ...
    private func toolIconView(for tool: String) -> some View {
        ...
        case "MCP":
            Image(systemName: "desktopcomputer")
                .baoziFont(size: toolLogFontSize - 1, weight: .semibold)
        case "Tool":
            Image(systemName: "wrench.and.screwdriver")
                .baoziFont(size: toolLogFontSize - 1, weight: .semibold)
        case "Bash":
            Text("$").baoziFont(size: toolLogFontSize - 1, weight: .semibold)
        case "Edit":
            Text("✎").baoziFont(size: toolLogFontSize - 1, weight: .semibold)
    ...
    private var responsePreview: some View {
        ...
        // naturally updates at ~8Hz instead of forcing `BaoziMarkdownView`
    ...
    private func forkRune(lineage: ThreadLineage) -> some View {
        ...
        .overlay(
            Capsule()
                .stroke(BaoziTheme.border.opacity(0.6), lineWidth: 1)
        )
        .clipShape(Capsule())
    ...
    private func branchChip(lineage: ThreadLineage) -> some View {
        HStack(spacing: 3) {
            Image(systemName: "arrow.triangle.branch")
                .baoziFont(size: 7, weight: .semibold)
        ...
        if let lineage = session.lineage, !lineage.ancestors.isEmpty {
            HStack(spacing: 0) {
                ForEach(Array(lineage.ancestors.enumerated()), id: \.offset) { idx, ancestor in
                    if idx > 0 {
                        Text(" › ")
                            .foregroundStyle(BaoziTheme.textMuted.opacity(0.55))
                    }
                    Text(ancestor.title)
                        .lineLimit(1)
                        .truncationMode(.tail)
                        .foregroundStyle(BaoziTheme.textMuted.opacity(0.85))
                }
                Text(" ›")
                    .foregroundStyle(BaoziTheme.textMuted.opacity(0.55))
                Spacer(minLength: 0)
            }
            .baoziMonoFont(size: 9, weight: .regular)
            .padding(.bottom, 2)
        }
    }
    ...
    private func siblingPill(member: ThreadLineageMember, isCurrent: Bool) -> some View {
        HStack(spacing: 5) {
            Circle()
                .fill(isCurrent ? BaoziTheme.accent : BaoziTheme.textMuted.opacity(0.5))
                .frame(width: 5, height: 5)
            Text(member.title)
                .lineLimit(1)
                .truncationMode(.tail)
        }
        .baoziFont(size: 10, weight: isCurrent ? .semibold : .regular)
        .foregroundStyle(isCurrent ? BaoziTheme.accent : BaoziTheme.textSecondary.opacity(0.85))
        .padding(.horizontal, 8)
        .padding(.vertical, 3)
        .background(
            Capsule()
                .fill(isCurrent ? BaoziTheme.accent.opacity(0.12) : BaoziTheme.surface.opacity(0.6))
        )
        .overlay(
            Capsule()
                .stroke(isCurrent ? BaoziTheme.accent.opacity(0.6) : BaoziTheme.border.opacity(0.6), lineWidth: 1)
        )
    }
}
...
/// message bodies (`BaoziFont.conversationBodyPointSize × textScale`) so
...
private struct MarkdownMatchedTitleFont: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .font(.custom(
                BaoziFont.markdownFontName,
                size: BaoziFont.conversationBodyPointSize * textScale
            ))
            .fontWeight(.medium)
    }
}
