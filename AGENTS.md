...
- **alleycat fork**: the Rust deps in `shared/rust-bridge/**/Cargo.toml` point at `huangguang1999/alleycat`. Fresh Rust builds require that fork to exist; until it is pushed, temporarily revert those deps to `dnakov/alleycat`. (`ish-embed-host` legitimately stays on `dnakov/litter-ish` — an upstream dep, not forked.)
