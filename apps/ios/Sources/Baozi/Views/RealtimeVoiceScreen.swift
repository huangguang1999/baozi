...
struct RealtimeVoiceScreen: View {
    ...
    private var promptFillColor: Color {
        BaoziTheme.surface.opacity(colorScheme == .dark ? 0.82 : 0.92)
    }

    private var promptStrokeColor: Color {
        BaoziTheme.border.opacity(colorScheme == .dark ? 0.55 : 0.8)
    }

    private var controlFillColor: Color {
        BaoziTheme.surfaceLight.opacity(colorScheme == .dark ? 0.72 : 0.88)
    }
        ...
        VStack(spacing: 16) {
            HStack(spacing: 8) {
                ...
                Text(phase.displayTitle.uppercased())
                    .font(BaoziFont.monospaced(.caption, weight: .bold))
    ...
    private func transcriptLine(
        ...
        return Text(entry.text)
            .font(BaoziFont.styled(textStyle, weight: fontWeight))
    ...
    private var realtimeApiKeyPrompt: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("Realtime needs an API key")
                .font(BaoziFont.styled(.headline, weight: .semibold))
                .foregroundColor(primaryTextColor)

            Text("Enter your OpenAI API key for this device. Baozi will store it in the local Codex environment as OPENAI_API_KEY.")
                .font(BaoziFont.styled(.caption))
                .foregroundColor(secondaryTextColor)
                .fixedSize(horizontal: false, vertical: true)
            ...
            SecureField("sk-...", text: $apiKey)
                ...
                .font(BaoziFont.monospaced(.body))
                .foregroundColor(primaryTextColor)
    ...
    private var apiKeySaveButtonLabel: some View {
        HStack(spacing: 10) {
            ...
            Text(isSavingApiKey ? "Saving…" : "Save API Key")
                .font(BaoziFont.styled(.subheadline, weight: .semibold))
        }
        .foregroundColor(primaryTextColor)
        .frame(maxWidth: .infinity)
        .padding(.vertical, 12)
