...
/// payload (Task #7). The shared types live in the BaoziWatchComplications
...
final class PerServerComplicationTests: XCTestCase {
    ...
    func testServerListPayloadRoundTripsThroughJSON() throws {
        let payload = BaoziServerListPayload(servers: [
            .init(id: "macbook", displayName: "MacBook Pro"),
            .init(id: "studio", displayName: "Mac Studio"),
        ])

        let data = try JSONEncoder().encode(payload)
        let decoded = try JSONDecoder().decode(BaoziServerListPayload.self, from: data)
    ...
    func testPerServerSnapshotsFilterRunningTaskToOwningServer() throws {
        ...
        let macbook = try JSONDecoder().decode(BaoziComplicationPayload.self, from: macbookData)
        ...
        let studio = try JSONDecoder().decode(BaoziComplicationPayload.self, from: studioData)
    ...
    func testPerServerSnapshotsEmitOfflineWhenWatchUnreachable() throws {
        ...
        let decoded = try JSONDecoder().decode(BaoziComplicationPayload.self, from: data)
