...
struct ConversationComposerModalCoordinator<Content: View>: View {
    ...
    var body: some View {
        content
            ...
            .sheet(isPresented: $showModelSelector) {
                ...
                .presentationBackground(BaoziTheme.surface)
            }
                ...
                VStack(alignment: .leading, spacing: 14) {
                    VStack(alignment: .leading, spacing: 12) {
                        HStack(alignment: .center) {
                            ...
                            Text(currentRuntimeSupportsPermissionOverrides ? (usesThreadDefaults ? "Using defaults" : "Custom override") : "Runtime managed")
                                .foregroundStyle(usesThreadDefaults ? BaoziTheme.textSecondary : BaoziTheme.accentStrong)
                                .baoziFont(size: 11, weight: .semibold)
                                .padding(.horizontal, 10)
                                .padding(.vertical, 6)
                                .background(
                                    Capsule()
                                        .fill((usesThreadDefaults ? BaoziTheme.surfaceLight : BaoziTheme.accentStrong).opacity(0.16))
                                )
                        }
                        ...
                        HStack(spacing: 10) {
                            ...
                            permissionSummaryTile(
                                ...
                                accent: hasAuthoritativeThreadPermissions ? BaoziTheme.textSecondary : BaoziTheme.warning
                            )
                        }
                    }
                    .padding(14)
                    .background(
                        RoundedRectangle(cornerRadius: 20, style: .continuous)
                            .fill(BaoziTheme.surface.opacity(0.82))
                    )
                    .overlay(
                        RoundedRectangle(cornerRadius: 20, style: .continuous)
                            .stroke(BaoziTheme.border.opacity(0.55), lineWidth: 1)
                    )
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 8) {
                Image(systemName: "info.circle.fill")
                    .foregroundStyle(BaoziTheme.accentStrong)
                Text("Runtime-managed permissions")
                    .foregroundStyle(BaoziTheme.textPrimary)
                    .baoziFont(.subheadline, weight: .semibold)
            }
            Text("This agent does not support Baozi-side thread permission overrides, so approval and sandbox choices are not sent for this session.")
                .foregroundStyle(BaoziTheme.textMuted)
                .baoziFont(.caption)
        }
        .padding(14)
    ...
    private func permissionSummaryTile(
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(BaoziTheme.surfaceLight.opacity(0.78))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .stroke(BaoziTheme.border.opacity(0.45), lineWidth: 1)
        )
    }
    ...
    private func permissionSection<SectionContent: View>(
        ...
        .background(
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .fill(BaoziTheme.surface.opacity(0.74))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .stroke(BaoziTheme.border.opacity(0.5), lineWidth: 1)
        )
    }
        ...
        Menu {
            ...
            .background(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .fill(BaoziTheme.surfaceLight.opacity(0.9))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .stroke(BaoziTheme.border.opacity(0.45), lineWidth: 1)
