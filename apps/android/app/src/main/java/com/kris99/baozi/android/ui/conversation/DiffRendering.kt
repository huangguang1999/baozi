...
internal fun SyntaxHighlightedDiffBlock(
    ...
    val resolvedFontPx: Float = if (fontSize.isUnspecified) {
        BaoziTextStyle.caption * textScale
    } else {
        fontSize.value * textScale
    }
    val palette = remember(
        BaoziTheme.textBody.toArgb(),
        BaoziTheme.textSecondary.toArgb(),
        BaoziTheme.success.toArgb(),
        BaoziTheme.danger.toArgb(),
        BaoziTheme.accentStrong.toArgb(),
        BaoziTheme.codeBackground.toArgb(),
        BaoziTheme.surface.copy(alpha = 0.72f).toArgb(),
    ) {
        DiffSyntaxPalette(
            context = BaoziTheme.textBody.toArgb(),
            metadata = BaoziTheme.textSecondary.toArgb(),
            addition = BaoziTheme.success.toArgb(),
            deletion = BaoziTheme.danger.toArgb(),
            hunk = BaoziTheme.accentStrong.toArgb(),
            contextBackground = BaoziTheme.codeBackground.toArgb(),
            metadataBackground = BaoziTheme.surface.copy(alpha = 0.72f).toArgb(),
            additionBackground = BaoziTheme.success.copy(alpha = 0.12f).toArgb(),
            deletionBackground = BaoziTheme.danger.copy(alpha = 0.12f).toArgb(),
            hunkBackground = BaoziTheme.accentStrong.copy(alpha = 0.12f).toArgb(),
        )
    }
    ...
    AndroidView(
        ...
        update = { scrollView ->
            ...
            textView.setTextColor(BaoziTheme.textBody.toArgb())
