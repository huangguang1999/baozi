                ...
                ForEach(files) { file in
                    HStack(spacing: 5) {
                        ...
                        Button {
                            ...
                            Image(systemName: "xmark")
                                .baoziFont(size: 9, weight: .bold)
                                .foregroundStyle(BaoziTheme.accent)
                                .padding(3)
                                .background(Circle().fill(BaoziTheme.accent.opacity(0.18)))
                        }
                        .buttonStyle(.plain)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 8, style: .continuous)
                            .fill(BaoziTheme.surfaceLight.opacity(0.72))
                    )
                }
            }
        }
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(plugins, id: \.path) { plugin in
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 8, style: .continuous)
                            .fill(BaoziTheme.accent.opacity(0.12))
                    )
                }
            }
        }
...
struct ConversationComposerModeChip: View {
    ...
    private var foreground: Color {
        mode == .plan ? Color.black : BaoziTheme.textPrimary
    }

    private var background: Color {
        mode == .plan ? BaoziTheme.accent : BaoziTheme.surfaceLight
    }
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(BaoziTheme.codeBackground.opacity(0.92))
        )
    }
...
private struct ConversationComposerGoalRowView: View {
    ...
    private var statusPill: some View {
        Button(action: { if canTogglePause { actions.togglePause() } }) {
            HStack(spacing: 5) {
                ...
                Text(statusLabel)
                    .baoziMonoFont(size: 10, weight: .semibold)
    ...
    private func budgetGauge(progress: Double) -> some View {
        ...
        return HStack(spacing: 8) {
            ...
            HStack(spacing: 4) {
                ...
                Text("\(percent)%")
                    .baoziMonoFont(size: 10, weight: .bold)
    ...
    private var progressTextTint: Color {
        guard let progress = budgetProgress else { return BaoziTheme.textSecondary }
        if progress >= 1.0 { return BaoziTheme.danger }
        if progress >= 0.85 { return BaoziTheme.warning }
    ...
    private var usageMetricsRow: some View {
        HStack(spacing: 6) {
            ...
            if goal.tokensUsed > 0 && goal.timeUsedSeconds > 0 {
                Text("·")
                    .baoziMonoFont(size: 10, weight: .semibold)
                    .foregroundColor(BaoziTheme.textMuted.opacity(0.6))
            }
        ...
        if #available(iOS 26.0, *) {
            ...
            content
                .background(
                    LinearGradient(
                        colors: [
                            BaoziTheme.codeBackground.opacity(0.92),
