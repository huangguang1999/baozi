...
fun markdownMatchedTitleStyle(): TextStyle {
    val family = if (BaoziThemeManager.monoFontEnabled) BaoziTheme.monoFont else FontFamily.Default
    return TextStyle(
        fontFamily = family,
...
object HomeDashboardSupport {
     ...
     * unless it is the placeholder "未命名会话", otherwise fall back
    ...
    fun sessionTitle(session: AppSessionSummary): String {
        ...
        if (trimmed.isNotEmpty() && trimmed != "未命名会话") return trimmed
