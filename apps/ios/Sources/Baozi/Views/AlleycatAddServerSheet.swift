...
struct AlleycatAddServerSheet: View {
        ...
        NavigationStack {
            ZStack {
                ...
                Form {
                    ...
                    if let parseError {
                        errorSection(parseError, color: BaoziTheme.warning)
                    }
                    if let agentError {
                        errorSection(agentError, color: BaoziTheme.warning)
                    }
                    connectSection
                    if let connectError {
                        errorSection(connectError, color: BaoziTheme.danger)
                    }
                }
    ...
    private func requestInitialScanIfNeeded() {
        guard !BaoziPlatform.rendersAsMacApp else { return }
    ...
    private static let pairCommandLabel = "npx baozicli"
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                HStack(spacing: 6) {
                                    Text(agent.displayName)
                                        .baoziFont(.subheadline)
                                        .foregroundColor(agent.available ? BaoziTheme.textPrimary : BaoziTheme.textMuted)
...
private struct QRScannerScreen: View {
    ...
    private static let pairCommand = "npx baozicli"
    ...
    private func stepRow(number: String, title: String) -> some View {
        HStack(alignment: .top, spacing: 10) {
            Text(number)
                ...
                .background(BaoziTheme.accent, in: Circle())
            Text(title)
                .font(.system(size: 13))
