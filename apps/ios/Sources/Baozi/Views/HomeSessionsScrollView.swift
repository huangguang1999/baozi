...
final class HomeSessionsScrollUIView: UIView {
    ...
    static let homeBecameActive = Notification.Name("baoziHomeBecameActive")
...
final class HomeRowContainer: UIView {
    ...
    init(scrollHost: HomeSessionsScrollUIView) {
        ...
        pinchBlur.alpha = 1
        if !BaoziPlatform.rendersAsMacApp {
        ...
        pinchHighlight.backgroundColor = UIColor(BaoziTheme.accent).withAlphaComponent(0.14)
    ...
    func setPinchBlurProgress(_ progress: CGFloat) {
        ...
        if BaoziPlatform.rendersAsMacApp { return }
    ...
    private func updateActionsVisuals() {
        ...
        if offsetX > 0 {
            actionsBackground.backgroundColor = UIColor(BaoziTheme.accent)
            ...
            actionsBackground.backgroundColor = UIColor(BaoziTheme.danger)
            actionsBackground.alpha = tintAlpha
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(session.sessionTitle.isEmpty ? "Session" : session.sessionTitle)
                .baoziFont(size: BaoziFont.conversationBodyPointSize, weight: .medium)
            ...
            if !session.serverDisplayName.isEmpty {
                HStack(spacing: 5) {
                    Text(session.agentRuntimeKind.displayLabel)
                        ...
                        .foregroundStyle(BaoziTheme.accent.opacity(0.8))
                    Text(session.serverDisplayName)
                        .baoziMonoFont(size: 10)
                        .foregroundStyle(BaoziTheme.textSecondary.opacity(0.75))
                        .lineLimit(1)
                }
            }
        }
