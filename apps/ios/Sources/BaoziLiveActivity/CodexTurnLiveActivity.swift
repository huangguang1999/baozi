...
struct CodexTurnLiveActivity: Widget {
    ...
    private var warningColor: Color { BaoziPalette.warning.color(for: .dark) }
    private var dangerColor: Color { BaoziPalette.danger.color(for: .dark) }
    private var primaryText: Color { BaoziPalette.textPrimary.color(for: .dark) }
    private var secondaryText: Color { BaoziPalette.textSecondary.color(for: .dark) }
    private var mutedText: Color { BaoziPalette.textMuted.color(for: .dark) }
                ...
                DynamicIslandExpandedRegion(.center) {
                    VStack(alignment: .leading, spacing: 3) {
                        Text(context.attributes.prompt)
                            .font(.system(size: 11, weight: .medium, design: BaoziPalette.fontDesign))
                        ...
                        HStack(spacing: 6) {
                            ...
                            if context.state.toolCallCount > 0 {
                                Label("\(context.state.toolCallCount)", systemImage: "chevron.left.forwardslash.chevron.right")
                                    .font(.system(size: 9, design: BaoziPalette.fontDesign))
    ...
    private func baoziLogo(size: CGFloat) -> some View {
        Image("brand_logo")
            .resizable()
            .interpolation(.high)
            .scaledToFit()
    ...
    private func phaseBadge(_ state: CodexTurnAttributes.ContentState) -> some View {
        Text(phaseBadgeText(state))
            .font(.system(size: 10, weight: .medium, design: BaoziPalette.fontDesign))
    ...
    private func metaChip(systemImage: String, text: String) -> some View {
        HStack(spacing: 4) {
            ...
            Text(text)
                .font(.system(size: 10, design: BaoziPalette.fontDesign))
                .foregroundStyle(mutedText)
                .lineLimit(1)
        }
        .padding(.trailing, 10)
    ...
    private func ctxBadge(_ percent: Int) -> some View {
        Text("\(percent)%")
            .font(.system(size: 9, weight: .semibold, design: BaoziPalette.fontDesign))
    ...
    private func liveTimer(context: ActivityViewContext<CodexTurnAttributes>, size: CGFloat) -> some View {
        if isActive(context.state) {
            Text(timerInterval: context.attributes.startDate...Date.distantFuture, countsDown: false)
                .font(.system(size: size, design: BaoziPalette.fontDesign))
                .monospacedDigit()
...
private struct AdaptiveLiveTimer: View {
    ...
    private var timerColor: Color {
        let text = BaoziPalette.textSecondary.color(for: colorScheme)
    ...
    var body: some View {
        ...
        .font(.system(size: 15, design: BaoziPalette.fontDesign))
        .monospacedDigit()
        .fontWeight(.regular)
