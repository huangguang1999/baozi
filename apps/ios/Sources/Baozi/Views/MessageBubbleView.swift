...
enum BaoziMarkdownStyleVariant {
    case content
    case system
}

struct BaoziMarkdownView: View {
    let markdown: String
    var style: BaoziMarkdownStyleVariant = .content
    var bodySize: CGFloat = BaoziFont.conversationBodyPointSize
    var codeSize: CGFloat = BaoziFont.conversationBodyPointSize
    ...
    var body: some View {
        if debugSettings.enabled && debugSettings.disableMarkdown {
            Text(markdown)
                ...
                .foregroundColor(style == .system ? BaoziTheme.textSecondary : BaoziTheme.textPrimary)
    ...
    private func renderedMarkdown(selectionEnabled: Bool) -> some View {
        ...
        switch style {
        case .content:
            view.baoziContentMarkdown(
                bodySize: bodySize, codeSize: codeSize,
                selectionEnabled: selectionEnabled
            )
        case .system:
            view.baoziSystemMarkdown(
                bodySize: bodySize, codeSize: codeSize,
                selectionEnabled: selectionEnabled
            )
        }
...
private extension BaoziMarkdownStyleVariant {
    var cacheKey: String {
        switch self {
        case .content:
            return "content"
...
struct UserBubble: View {
    ...
    private let contentFontSize = BaoziFont.conversationBodyPointSize
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 0) {
            ...
            .modifier(GlassRectModifier(cornerRadius: compact ? 14 : 18, tint: BaoziTheme.accent.opacity(0.3)))
        }
        .padding(.bottom, 14)
...
private struct BaoziMathBlockView: View {
    let latex: String
...
struct StreamingAssistantBubble: View {
    ...
    init(
        ...
        label: String? = nil,
        themeVersion: Int = 0,
        bodySize: CGFloat = BaoziFont.conversationBodyPointSize,
    ...
    private var genericSystemBubble: some View {
        ...
        .overlay(alignment: .leading) {
            RoundedRectangle(cornerRadius: 1)
                .fill(BaoziTheme.accent.opacity(0.9))
                .frame(width: 3)
                .padding(.vertical, 6)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
...
// MARK: - Baozi Markdown Themes

private func baoziContentTheme(bodySize: CGFloat, codeSize: CGFloat) -> MarkdownTheme {
    var theme = MarkdownTheme.default
    theme.bodyFont = .custom(BaoziFont.markdownFontName, size: bodySize)
    theme.bodyFontSize = bodySize
    theme.foregroundColor = BaoziTheme.textBody
    ...
    theme.headingStyleSet = HeadingStyleSet(
        h1: HeadingStyle(fontSize: bodySize * 1.43, weight: .bold,
                         topSpacing: 16, bottomSpacing: 8, color: BaoziTheme.textPrimary),
        h2: HeadingStyle(fontSize: bodySize * 1.21, weight: .semibold,
                         topSpacing: 12, bottomSpacing: 6, color: BaoziTheme.textPrimary),
        h3: HeadingStyle(fontSize: bodySize * 1.07, weight: .semibold,
                         topSpacing: 10, bottomSpacing: 4, color: BaoziTheme.textPrimary),
        h4: HeadingStyle(fontSize: bodySize, weight: .semibold, color: BaoziTheme.textPrimary),
        h5: HeadingStyle(fontSize: bodySize, weight: .semibold, color: BaoziTheme.textPrimary),
        h6: HeadingStyle(fontSize: bodySize, weight: .semibold, color: BaoziTheme.textPrimary)
    )

    theme.inlineCode = InlineCodeStyle(
        backgroundColor: BaoziTheme.surfaceLight,
        textColor: BaoziTheme.textPrimary,
        font: .custom(BaoziFont.markdownFontName, size: codeSize),
        fontSize: codeSize
    )

    theme.codeBlock = CodeBlockStyle(
        backgroundColor: BaoziTheme.codeBackground.opacity(0.8),
        textColor: BaoziTheme.textPrimary,
        font: .custom(BaoziFont.markdownFontName, size: codeSize),
        fontSize: codeSize,
        cornerRadius: 8,
    ...
    theme.blockquote = BlockquoteStyle(
        borderColor: BaoziTheme.border,
        borderWidth: 3,
        textColor: BaoziTheme.textSecondary,
        padding: EdgeInsets(top: 8, leading: 12, bottom: 8, trailing: 4)
    )

    theme.table = TableStyle(
        borderStyle: .solid(color: BaoziTheme.border, width: 0.5),
        headerBackground: BaoziTheme.surfaceLight,
        headerFontWeight: .semibold,
        backgroundStyle: .alternatingRows(
            even: BaoziTheme.surface.opacity(0.5),
            odd: .clear
        ),
        cornerRadius: 8
    )
    ...
    theme.link = LinkStyle(color: BaoziTheme.accent, underline: false)
...
private func baoziSystemTheme(bodySize: CGFloat, codeSize: CGFloat) -> MarkdownTheme {
    var theme = MarkdownTheme.default
    theme.bodyFont = .custom(BaoziFont.markdownFontName, size: bodySize)
    theme.bodyFontSize = bodySize
    theme.foregroundColor = BaoziTheme.textSystem
    ...
    theme.headingStyleSet = HeadingStyleSet(
        ...
        h3: HeadingStyle(fontSize: bodySize * 1.08, weight: .semibold,
                         topSpacing: 8, bottomSpacing: 4, color: BaoziTheme.textPrimary),
        h4: HeadingStyle(fontSize: bodySize, weight: .semibold, color: BaoziTheme.textPrimary),
        h5: HeadingStyle(fontSize: bodySize, weight: .semibold, color: BaoziTheme.textPrimary),
        h6: HeadingStyle(fontSize: bodySize, weight: .semibold, color: BaoziTheme.textPrimary)
    )
...
struct BaoziCodeBlockRenderer: CodeBlockRenderer {
    @ViewBuilder
...
/// Maps a Baozi theme slug to the closest Highlightr theme name.
...
/// Returns the current Highlightr theme name based on the active Baozi theme.
...
/// Syncs the shared highlighter to match the current Baozi theme.
...
private struct ScaledContentMarkdownModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        let themed = content
            .markdownTheme(baoziContentTheme(bodySize: scaledBody, codeSize: scaledCode))
            .codeSyntaxHighlighter(sharedHighlighter)
            .codeBlockRenderer(BaoziCodeBlockRenderer())
        if selectionEnabled {
...
private struct ScaledSystemMarkdownModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        let _ = syncHighlighterTheme(for: colorScheme)
        let themed = content
            .markdownTheme(baoziSystemTheme(bodySize: scaledBody, codeSize: scaledCode))
            .codeSyntaxHighlighter(sharedHighlighter)
            .codeBlockRenderer(BaoziCodeBlockRenderer())
        if selectionEnabled {
            themed.textSelection(.enabled)
...
extension View {
    func baoziContentMarkdown(
        bodySize: CGFloat = BaoziFont.conversationBodyPointSize,
        codeSize: CGFloat = BaoziFont.conversationBodyPointSize,
    ...
    BaoziPreviewScene {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 12) {
                ForEach(BaoziPreviewData.sampleMessages) { message in
                    MessageBubbleView(
                        message: message,
                        serverId: BaoziPreviewData.sampleServer.id
                    )
                }
            }
            .padding(16)
