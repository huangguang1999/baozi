    ...
    var body: some View {
        ...
        .overlay(
            Capsule(style: .continuous)
                .stroke(
                    isSelected ? BaoziTheme.accent.opacity(0.75) : BaoziTheme.textMuted.opacity(0.25),
...
private struct AgentRuntimeOverflowBadge: View {
    ...
    var body: some View {
        Text("+\(count)")
            .baoziMonoFont(size: 9, weight: .bold)
            .foregroundStyle(BaoziTheme.textPrimary)
            .lineLimit(1)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 5, style: .continuous)
                    .stroke(BaoziTheme.textPrimary.opacity(0.28), lineWidth: 0.55)
            )
