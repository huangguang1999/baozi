...
private const val THEME_LOG_TAG = "BaoziThemeManager"
private const val UI_PREFERENCES_NAME = "baozi_ui_prefs"
...
enum class BaoziAppearanceMode(
    ...
    companion object {
        fun fromStorageValue(value: String?): BaoziAppearanceMode? =
...
enum class BaoziColorThemeType {
    LIGHT,
    DARK,
}

data class BaoziThemeIndexEntry(
    val slug: String,
    val name: String,
    val type: BaoziColorThemeType,
    val accentHex: String,
    val backgroundHex: String,
    val foregroundHex: String,
)

data class BaoziThemeDefinition(
    val name: String,
    val type: BaoziColorThemeType,
    val colors: Map<String, String>,
)

data class BaoziResolvedTheme(
    val slug: String,
    val name: String,
    val type: BaoziColorThemeType,
    ...
    companion object {
        val defaultLight =
            resolve(
                slug = "codex-light",
                definition =
                    BaoziThemeDefinition(
                        name = "Codex Light",
                        type = BaoziColorThemeType.LIGHT,
                        colors =
                            mapOf(
        ...
        fun resolve(
            slug: String,
            definition: BaoziThemeDefinition,
        ): BaoziResolvedTheme {
            val colors = definition.colors
            val background =
                colorFromHex(
                    colors["editor.background"],
                    fallback = if (definition.type == BaoziColorThemeType.DARK) Color(0xFF111111) else Color.White,
                )
            val foreground =
                colorFromHex(
                    colors["editor.foreground"],
                    fallback = if (definition.type == BaoziColorThemeType.DARK) Color(0xFFFCFCFC) else Color(0xFF0D0D0D),
                )
            val surface =
                colors["sideBar.background"]?.let(::colorFromHex)
                    ?: adjustBrightness(background, if (definition.type == BaoziColorThemeType.DARK) 0.03f else -0.02f)
            val surfaceLight =
                colors["activityBar.background"]?.let(::colorFromHex)
                    ?: adjustBrightness(surface, if (definition.type == BaoziColorThemeType.DARK) 0.04f else -0.03f)
            val accent =
                colors["textLink.foreground"]?.let(::colorFromHex)
                    ?: colors["button.background"]?.let(::colorFromHex)
                    ?: if (definition.type == BaoziColorThemeType.DARK) Color(0xFFB0B0B0) else Color(0xFF4A4A4A)
            val accentStrong =
                colors["button.background"]?.let(::colorFromHex)
                    ?: colors["textLink.foreground"]?.let(::colorFromHex)
                    ?: accent
            val border =
                colors["editorGroup.border"]?.let(::colorFromHex)
                    ...
                    ?: adjustBrightness(surface, if (definition.type == BaoziColorThemeType.DARK) 0.05f else -0.05f)
            val separator =
                colors["panel.border"]?.let(::colorFromHex)
                    ?: adjustBrightness(background, if (definition.type == BaoziColorThemeType.DARK) 0.04f else -0.04f)

            return BaoziResolvedTheme(
                slug = slug,
                ...
                border = border,
                separator = separator,
                danger = if (definition.type == BaoziColorThemeType.DARK) Color(0xFFFF5555) else Color(0xFFD32F2F),
                success = if (definition.type == BaoziColorThemeType.DARK) Color(0xFF6EA676) else Color(0xFF2E7D32),
                warning = if (definition.type == BaoziColorThemeType.DARK) Color(0xFFE2A644) else Color(0xFFE65100),
...
object BaoziThemeManager {
    private val lock = Any()
    private var appContext: Context? = null
    private var initialized = false
    private var definitionCache = LinkedHashMap<String, BaoziThemeDefinition>()
    private var systemIsDark = false

    var appearanceMode by mutableStateOf(BaoziAppearanceMode.SYSTEM)
        private set

    var monoFontEnabled by mutableStateOf(true)
        private set

    var lightTheme by mutableStateOf(BaoziResolvedTheme.defaultLight)
        private set

    var darkTheme by mutableStateOf(BaoziResolvedTheme.defaultDark)
        private set

    var activeTheme by mutableStateOf(BaoziResolvedTheme.defaultDark)
        private set

    var themeVersion by mutableIntStateOf(0)
        private set

    var themeIndex by mutableStateOf<List<BaoziThemeIndexEntry>>(emptyList())
        private set

    val lightThemes: List<BaoziThemeIndexEntry>
        get() = themeIndex.filter { it.type == BaoziColorThemeType.LIGHT }

    val darkThemes: List<BaoziThemeIndexEntry>
        get() = themeIndex.filter { it.type == BaoziColorThemeType.DARK }

    val selectedLightSlug: String
        get() = preferences?.getString(SELECTED_LIGHT_THEME_KEY, null) ?: "baozi-light"

    val selectedDarkSlug: String
        get() = preferences?.getString(SELECTED_DARK_THEME_KEY, null) ?: "baozi-dark"
    ...
    fun initialize(context: Context) {
        synchronized(lock) {
            ...
            lightTheme = loadAndResolve(selectedLightSlug) ?: BaoziResolvedTheme.defaultLight
            darkTheme = loadAndResolve(selectedDarkSlug) ?: BaoziResolvedTheme.defaultDark
    ...
    fun applyDarkMode(enabled: Boolean) {
        applyAppearanceMode(if (enabled) BaoziAppearanceMode.DARK else BaoziAppearanceMode.LIGHT)
    }

    fun applyAppearanceMode(mode: BaoziAppearanceMode) {
    ...
    fun selectLightTheme(slug: String) {
        ...
        lightTheme = loadAndResolve(slug) ?: BaoziResolvedTheme.defaultLight
        if (!usesDarkTheme()) {
    ...
    fun selectDarkTheme(slug: String) {
        ...
        darkTheme = loadAndResolve(slug) ?: BaoziResolvedTheme.defaultDark
        if (usesDarkTheme()) {
    ...
    private fun loadAppearanceMode(): BaoziAppearanceMode {
        val prefs = preferences ?: return BaoziAppearanceMode.SYSTEM
        BaoziAppearanceMode.fromStorageValue(prefs.getString(APPEARANCE_MODE_KEY, null))?.let {
            return it
        }
        return if (prefs.contains(DARK_MODE_KEY)) {
            if (prefs.getBoolean(DARK_MODE_KEY, false)) {
                BaoziAppearanceMode.DARK
            } else {
                BaoziAppearanceMode.LIGHT
            }
        } else {
            BaoziAppearanceMode.SYSTEM
        }
    }

    private fun usesDarkTheme(mode: BaoziAppearanceMode = appearanceMode): Boolean =
        mode.resolvesDarkTheme(systemIsDark)

    private fun themeForMode(mode: BaoziAppearanceMode): BaoziResolvedTheme =
    ...
    private fun loadThemeIndex(): List<BaoziThemeIndexEntry> {
    ...
    private fun loadAndResolve(slug: String): BaoziResolvedTheme? {
        val definition = loadDefinition(slug) ?: return null
        return BaoziResolvedTheme.resolve(slug = slug, definition = definition)
    }

    private fun loadDefinition(slug: String): BaoziThemeDefinition? {
    ...
    private fun parseThemeDefinition(json: JSONObject): BaoziThemeDefinition {
...
private fun String.toThemeType(): BaoziColorThemeType =
    if (equals("light", ignoreCase = true)) {
        BaoziColorThemeType.LIGHT
    } else {
        BaoziColorThemeType.DARK
    }
