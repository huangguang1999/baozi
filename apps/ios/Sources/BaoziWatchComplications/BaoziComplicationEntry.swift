...
/// through the `group.com.kris99.baozi` App Group — the iOS app writes
/// the current running-task snapshot into `UserDefaults` and complications
/// read it on each reload.
struct BaoziComplicationEntry: TimelineEntry {
    ...
    static let placeholder = BaoziComplicationEntry(
        date: .now,
        mode: .running,
    ...
    static let idlePlaceholder = BaoziComplicationEntry(
        date: .now,
        mode: .idle,
...
struct BaoziComplicationPayload: Codable, Equatable {
    let mode: BaoziComplicationEntry.Mode
    let lastTurnStartMsEpoch: Int64?
    let taskId: String?
    let progress: Double
    let title: String
...
enum BaoziComplicationStore {
    ...
    static func entry(from payload: BaoziComplicationPayload, at date: Date = .now) -> BaoziComplicationEntry {
    ...
    static func write(_ entry: BaoziComplicationEntry) {
        guard let defaults = UserDefaults(suiteName: appGroup) else { return }
        let payload = BaoziComplicationPayload(
            mode: entry.mode,
