...
private struct ConversationTimelineItemRow: View, Equatable {
    ...
    private func fileChangeSummary(for data: ConversationFileChangeData) -> (plainText: String, attributedText: AttributedString?) {
        ...
        attributed.foregroundColor = BaoziTheme.textSystem
...
private struct ConversationExplorationGroupRow: View {
            ...
            Button(action: toggleExpanded) {
                HStack(spacing: 8) {
                    Image(systemName: "magnifyingglass")
                        ...
                        .foregroundColor(isActive ? BaoziTheme.warning : BaoziTheme.textSecondary)
            ...
            if expanded {
                VStack(alignment: .leading, spacing: 4) {
                    ForEach(entries) { entry in
                        HStack(alignment: .top, spacing: 8) {
                            Circle()
                                .fill(entry.isInProgress ? BaoziTheme.warning : BaoziTheme.textMuted)
                ...
                ScrollViewReader { proxy in
                    ...
                    .overlay(alignment: .top) {
                        LinearGradient(
                            colors: [BaoziTheme.surface.opacity(0.92), BaoziTheme.surface.opacity(0)],
                            startPoint: .top,
                            endPoint: .bottom
                        )
                        .frame(height: 16)
    ...
    private var collapsedPreviewHeight: CGFloat {
        (BaoziFont.uiMonoFont(size: 12 * textScale).lineHeight * 3) + 18
    }
...
private struct ConversationTodoListRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if expanded {
                        ...
                        ForEach(Array(data.steps.enumerated()), id: \.offset) { index, step in
                            HStack(alignment: .top, spacing: 8) {
                                ...
                                BaoziMarkdownView(
                                    ...
                                    .strikethrough(step.status == .completed, color: BaoziTheme.textMuted)
                ...
                .frame(maxHeight: 160)
                .background(BaoziTheme.surface.opacity(0.45))
                .mask {
                    VStack(spacing: 0) {
    ...
    private var headerTint: Color {
        if data.isComplete { return BaoziTheme.success }
        if hasInProgressStep { return BaoziTheme.warning }
    ...
    private var progressTint: Color {
        data.isComplete ? BaoziTheme.success : (hasInProgressStep ? BaoziTheme.warning : BaoziTheme.textSecondary)
    }
...
private struct ConversationCommandOutputViewport: View {
    ...
    private var maxViewportHeight: CGFloat {
        (BaoziFont.uiMonoFont(size: lineFontSize).lineHeight * 3) + 16
    }

    private var viewportHeight: CGFloat {
        let lh = BaoziFont.uiMonoFont(size: lineFontSize).lineHeight
    ...
    var body: some View {
        ScrollViewReader { proxy in
            VStack(alignment: .leading, spacing: 6) {
                ...
                .background(BaoziTheme.codeBackground.opacity(0.78))
                .clipShape(RoundedRectangle(cornerRadius: 10, style: .continuous))
                .overlay(alignment: .top) {
                    LinearGradient(
                        colors: [BaoziTheme.codeBackground.opacity(0.96), BaoziTheme.codeBackground.opacity(0)],
                        startPoint: .top,
                        endPoint: .bottom
                    )
                    .frame(height: 18)
                    ...
                    if let durationText, !durationText.isEmpty {
                        Text(durationText)
                            ...
                            .background(alignment: .bottom) {
                                LinearGradient(
                                    colors: [.clear, BaoziTheme.codeBackground.opacity(0.94)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            }
                ...
                .overlay {
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                        .stroke(BaoziTheme.border.opacity(0.35), lineWidth: 1)
                }
                .onAppear {
                    scrollToBottom(proxy)
                }
        ...
        case .contextCompaction:
            HStack(spacing: 6) {
                ...
                Text(title)
                    .baoziFont(.caption2, weight: .semibold)
                    .foregroundColor(
                        effectiveContextCompactionComplete ? BaoziTheme.textMuted : BaoziTheme.warning
                    )
                    .lineLimit(1)
            }
        default:
...
private struct ConversationCodeReviewFindingCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(alignment: .center, spacing: 10) {
                ...
                Button("Dismiss", action: onDismiss)
                    .buttonStyle(.plain)
                    .baoziFont(.callout, weight: .medium)
                    .foregroundColor(BaoziTheme.textSecondary)
            }

            BaoziMarkdownView(markdown: finding.body, style: .content, selectionEnabled: true)
        ...
        .padding(20)
        .background(BaoziTheme.surface.opacity(0.72), in: RoundedRectangle(cornerRadius: 22))
        .overlay(
                ...
                Button {
                    ...
                    HStack(spacing: 8) {
                        Image(systemName: completed == total && total > 0 ? "checkmark.circle.fill" : "checklist")
                            .baoziFont(size: 11, weight: .semibold)
                            .foregroundColor(completed == total && total > 0 ? BaoziTheme.success : BaoziTheme.accent)
                        Text(summary)
                            .baoziFont(.caption, weight: .semibold)
                            .foregroundColor(BaoziTheme.textPrimary)
                            .lineLimit(2)
...
private struct DiffIndicatorLabel: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, 10)
        .padding(.vertical, 8)
        .background(BaoziTheme.surface.opacity(0.72), in: Capsule())
        .fixedSize(horizontal: true, vertical: false)
        .accessibilityElement(children: .ignore)
...
private struct DiffLine: Identifiable {
    enum Kind {
        ...
        var foregroundColor: Color {
            switch self {
            case .addition: BaoziTheme.success
            case .deletion: BaoziTheme.danger
            case .hunk: BaoziTheme.accentStrong
            case .context: BaoziTheme.textBody
            }
        }

        var backgroundColor: Color {
            switch self {
            case .addition: BaoziTheme.success.opacity(0.12)
            case .deletion: BaoziTheme.danger.opacity(0.12)
            case .hunk: BaoziTheme.accentStrong.opacity(0.12)
            case .context: BaoziTheme.codeBackground.opacity(0.72)
            }
        }
    }
...
private struct ConversationDiffDetailSheet: View {
    ...
    private let fullDiffFontSize = BaoziFont.conversationDiffPointSize
