...
struct RateLimitBadgeView: View, Equatable {
    ...
    private var tint: Color {
        if percent <= 10 { return BaoziTheme.danger }
        if percent <= 30 { return BaoziTheme.warning }
    ...
    var body: some View {
        HStack(spacing: 3) {
            Text(label)
                .font(BaoziFont.monospaced(size: 9.5, weight: .semibold))
            ...
            ZStack(alignment: .bottomTrailing) {
                ScrollView {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        .frame(maxWidth: BaoziPlatform.isRegularSurface(horizontalSizeClass: horizontalSizeClass) ? 760 : .infinity)
    ...
    private var collapsedCard: some View {
        Button(action: onToggleExpansion) {
            previewTextBlock
                ...
                .modifier(GlassRectModifier(cornerRadius: 16, tint: BaoziTheme.surface.opacity(0.34)))
                .overlay(alignment: .bottomLeading) {
                    footerRow
                        .padding(.horizontal, 14)
                        .padding(.bottom, 10)
    ...
    private var previewTextBlock: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text(verbatim: responsePreviewText)
                .baoziFont(.body)
                .foregroundColor(BaoziTheme.textSecondary.opacity(0.82))
                .lineLimit(2)
                .truncationMode(.tail)
                .multilineTextAlignment(.leading)
                .frame(
    ...
    var body: some View {
        HStack(spacing: 4) {
            Image(systemName: systemImage)
                .baoziFont(size: 9, weight: .medium)
                .foregroundColor(BaoziTheme.textMuted)
            Text(verbatim: text)
                .baoziMonoFont(size: 10)
                .foregroundColor(BaoziTheme.textSecondary)
                .lineLimit(1)
        }
    }
...
private struct ConversationInputBar: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .baoziCommandSendComposer)) { _ in
            handleSend()
        }
        #endif
        .onDisappear {
        ...
        NavigationStack {
            List {
                if isLoading && presets.isEmpty {
                    ...
                    .listRowBackground(BaoziTheme.surface)
                }
                                ...
                                let optionButtons = ForEach(question.options, id: \.label) { option in
                                    ...
                                    Button {
                                        ...
                                        Text(option.label)
                                            ...
                                            .foregroundColor(isSelected ? Color.black : BaoziTheme.textPrimary)
                                            .padding(.horizontal, 10)
                                            .padding(.vertical, 6)
                                            .background(isSelected ? BaoziTheme.accent : BaoziTheme.surface.opacity(0.8))
                                            .clipShape(Capsule())
                                    }
                                    .buttonStyle(.plain)
                                }
...
struct QueuedFollowUpsPreviewView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                ...
                Text("\(previews.count)")
                    ...
                    .padding(.horizontal, 8)
                    .padding(.vertical, 4)
                    .background(BaoziTheme.surface.opacity(0.9))
                    .clipShape(Capsule())
            }
                ...
                HStack(alignment: .center, spacing: 12) {
                    ...
                    if preview.kind == .message || preview.kind == .pendingSteer {
                        Button(action: { onSteer(preview) }) {
                            ...
                            .foregroundColor(preview.kind == .pendingSteer ? BaoziTheme.accent : BaoziTheme.textPrimary)
                            .padding(.horizontal, 12)
                            .padding(.vertical, 8)
                            .background(BaoziTheme.surface.opacity(0.96))
                            .clipShape(Capsule())
                        }
                        .buttonStyle(.plain)
        ...
        .padding(12)
        .background(BaoziTheme.codeBackground.opacity(0.92))
        .clipShape(RoundedRectangle(cornerRadius: 14))
    }
}
...
private struct QueuedFollowUpPreviewStyle {
    ...
    static func forKind(_ kind: AppQueuedFollowUpKind) -> Self {
        ...
        case .pendingSteer:
            let tint = BaoziTheme.accentStrong
            return Self(
                title: "Steer queued",
    ...
    var body: some View {
        Text(label)
            ...
            .foregroundStyle(
                LinearGradient(
                    colors: [
                        BaoziTheme.textSecondary.opacity(0.4),
                        BaoziTheme.textSecondary.opacity(0.7),
                        BaoziTheme.textSecondary.opacity(0.4),
                    ],
    ...
    var body: some View {
        Button(action: action) {
            Image(systemName: "gamecontroller.fill")
                ...
                .background(
                    Circle()
                        .fill(BaoziTheme.surface.opacity(0.9))
                        .overlay(
                            Circle()
                                .stroke(BaoziTheme.accent.opacity(0.3), lineWidth: 0.5)
                        )
                )
...
private struct SubagentBreadcrumbBar: View {
    ...
    var body: some View {
        ...
        .background(
            BaoziTheme.surface.opacity(0.85)
        ...
        HStack(spacing: 6) {
            Button {
                ...
                Image(systemName: "ant")
                    ...
                    .background(
                        Circle()
                            .fill(BaoziTheme.surface.opacity(0.85))
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if debugSettings.enabled {
                Divider().background(BaoziTheme.border)
...
#Preview("Conversation") {
    BaoziPreviewScene(appModel: BaoziPreviewData.makeConversationAppModel(messages: BaoziPreviewData.longConversation)) {
        ContentView()
    }
}
#endif
