...
struct ThreadSearchResultsView: View {
    ...
    var body: some View {
        ScrollView {
            LazyVStack(spacing: 0) {
                ...
                if isLoading {
                    HStack(spacing: 8) {
                        ProgressView().controlSize(.small).tint(BaoziTheme.accent)
    ...
    private func runtimeFilterPill(label: String, kind: AgentRuntimeKind?) -> some View {
        ...
        return Button {
            ...
            .baoziFont(.caption)
            .foregroundStyle(isActive ? BaoziTheme.textOnAccent : BaoziTheme.textSecondary)
            ...
            HStack(alignment: .center, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    FormattedText(text: session.sessionTitle, lineLimit: 1)
                        .font(.custom(BaoziFont.markdownFontName, size: 13))
                        .fontWeight(.semibold)
                ...
                Image(systemName: isPinned ? "checkmark.circle.fill" : "plus.circle")
                    ...
                    .foregroundStyle(isPinned ? BaoziTheme.accent : BaoziTheme.textSecondary.opacity(0.7))
            }
            .padding(.horizontal, 14)
            .padding(.vertical, 8)
...
private struct ThreadSearchClusterRow: View {
    ...
    var body: some View {
        ...
        .background(isExpanded ? BaoziTheme.surface.opacity(0.3) : Color.clear)
    }
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                HStack(spacing: 4) {
                    ...
                    if let workspace = HomeDashboardSupport.workspaceLabel(for: session.cwd) {
                        ...
                        Text(workspace).foregroundStyle(BaoziTheme.textSecondary.opacity(0.8))
                    }
                    Text("\u{00b7}").foregroundStyle(BaoziTheme.textMuted.opacity(0.5))
        ...
        Button(action: onToggleExpanded) {
            HStack(spacing: 4) {
                ...
                Text("\(cluster.members.count)")
                    .baoziMonoFont(size: 11, weight: .semibold)
                    .foregroundStyle(BaoziTheme.textPrimary)
            ...
            .padding(.vertical, 4)
            .background(
                Capsule().fill(BaoziTheme.accent.opacity(isExpanded ? 0.18 : 0.12))
            )
            .overlay(
                Capsule().stroke(BaoziTheme.accent.opacity(0.4), lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
    ...
    private var childrenList: some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            ForEach(otherMembers) { member in
                ...
                Button(action: { isPinned ? onUnpin(member) : onPin(member) }) {
                    HStack(alignment: .center, spacing: 10) {
                        Rectangle()
                            .fill(BaoziTheme.accent.opacity(0.4))
                            .frame(width: 10, height: 1)
                        VStack(alignment: .leading, spacing: 1) {
                            HStack(spacing: 6) {
                                FormattedText(text: branchLabel(for: member, isRoot: isRoot), lineLimit: 1)
                                    .baoziFont(size: 12.5, weight: isPinned ? .semibold : .regular)
                                    .foregroundStyle(isPinned ? BaoziTheme.accent : BaoziTheme.textPrimary.opacity(0.92))
                                if isRoot {
                                    Text("root")
                            ...
                            Text(relativeDate(Int64(member.updatedAt.timeIntervalSince1970)))
                                .baoziMonoFont(size: 10, weight: .regular)
                                .foregroundStyle(BaoziTheme.textMuted.opacity(0.75))
                        }
                        Spacer(minLength: 6)
        ...
        .overlay(alignment: .leading) {
            Rectangle()
                .fill(BaoziTheme.accent.opacity(0.3))
                .frame(width: 2)
                .padding(.leading, 30)
        }
    }
