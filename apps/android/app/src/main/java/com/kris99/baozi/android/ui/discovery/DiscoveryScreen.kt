package com.kris99.baozi.android.ui.discovery
...
import com.kris99.baozi.android.state.connectionProgressDetail
...
import com.kris99.baozi.android.ui.common.BetaBadge
import com.kris99.baozi.android.ui.common.isBeta
...
import com.kris99.baozi.android.ui.common.metadata
...
fun DiscoveryScreen(
    ...
    fun startSlingshotConnect(environment: AppSlingshotEnvironment) {
        if (!environment.online) {
            connectError = "${environment.displayName} 已离线。"
            return
        }
        scope.launch {
            ...
            try {
                ...
                if (!ChatGPTOAuth.isRemoteControlAuthorizationRequired(e)) {
                    ...
                    connectError = e.message ?: "无法连接到此电脑。"
                    return@launch
                }
            ...
            try {
                ...
                pendingSlingshotEnvironment = null
                connectError = e.localizedMessage ?: e.message ?: "无法授权远程控制。"
            }
        }
    }
    ...
    Column(
        ...
        Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ChooserCard(
                title = "与 包子 配对",
                subtitle = "在电脑上运行 npx baozicli，然后扫描它打印的二维码。",
                badge = "推荐",
                icon = Icons.Default.QrCodeScanner,
                supportedAgents = BaoziAgents,
                isRecommended = true,
                ...
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    if (server.canConnectViaSsh) {
                        TextButton(
                            ...
                            Text("通过 SSH 连接", color = BaoziTheme.accent)
                        }
                    }
                }
            },
...
/**
 * Canonical agent list shown on the baozi chooser card. Mirrors
 ...
 * pre-pair preview.
 */
private val BaoziAgents: List<AgentRuntimeKind> = listOf(
    "codex",
    "pi",
    "amp",
    "opencode",
...
private fun ChooserCard(
    ...
    val borderColor = if (isRecommended) {
        ...
        BaoziTheme.accent.copy(alpha = 0.18f)
    }
    val backgroundColor = if (isRecommended) {
        BaoziTheme.surface.copy(alpha = 0.85f)
    } else {
        BaoziTheme.surface.copy(alpha = 0.6f)
    }
    val iconBubble = BaoziTheme.accent.copy(alpha = if (isRecommended) 0.16f else 0.10f)
                ...
                Row(
                    ...
                    if (badge != null) {
                        Box(
                            modifier = Modifier
                                .background(
                                    BaoziTheme.accent.copy(alpha = 0.14f),
    ...
    Row(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(5.dp)) {
            agents.forEach { agent ->
                com.kris99.baozi.android.ui.common.AgentIconView(
                    kind = agent,
                    sizeDp = 18,
                )
            }
...
private fun ServerRow(
    ...
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .background(BaoziTheme.surface, RoundedCornerShape(10.dp))
        ...
        Icon(
            ...
            tint = if (entry.hasCodexServer) BaoziTheme.accent else BaoziTheme.textMuted,
            modifier = Modifier.size(20.dp),
        )
        Spacer(Modifier.width(10.dp))
        Column(modifier = Modifier.weight(1f)) {
            Text(entry.name.ifBlank { entry.hostname }, color = BaoziTheme.textPrimary, fontSize = 14.sp)
            Text(subtitle, color = BaoziTheme.textSecondary, fontSize = 11.sp)
        }
        val (sourceColor, sourceLabel) = when (entry.source) {
            "bonjour" -> BaoziTheme.info to "Bonjour"
            "tailscale" -> Color(0xFFC797D8) to "Tailscale"
            "lanProbe" -> BaoziTheme.accent to "LAN"
            "arpScan" -> BaoziTheme.textSecondary to "ARP"
            "ssh" -> Color(0xFFFF9500) to "SSH"
            "local" -> BaoziTheme.accent to "本地"
            else -> BaoziTheme.textMuted to "手动"
        }
        Text(
            text = sourceLabel,
            color = sourceColor,
...
private fun ConnectedComputerRow(
    ...
    Row(
        ...
        Icon(
            ...
            contentDescription = null,
            tint = if (environment.online) BaoziTheme.accent else BaoziTheme.textMuted,
        ...
        Column(
            ...
            Text(
                ...
                color = if (environment.online) BaoziTheme.textPrimary else BaoziTheme.textSecondary,
                fontSize = 14.sp,
                fontWeight = FontWeight.SemiBold,
            )
            Text(
        ...
        Text(
            ...
            color = if (environment.online && !environment.busy) BaoziTheme.accent else BaoziTheme.textMuted,
            fontSize = 11.sp,
            fontWeight = FontWeight.SemiBold,
        )
    }
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    FilterChip(
                        ...
                        colors = FilterChipDefaults.filterChipColors(
                            selectedContainerColor = BaoziTheme.accent.copy(alpha = 0.18f),
                            selectedLabelColor = BaoziTheme.textPrimary,
                        ),
                    )
                    FilterChip(
